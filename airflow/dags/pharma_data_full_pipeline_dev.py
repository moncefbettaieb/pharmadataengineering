...
def get_last_pharma_scrapper_success(session=None, **context):
    """
    Récupère la dernière exécution réussie du task 'pharma_scrapper_dev', soustrait 3 jours.
    ...
    try:
        ...
        result = (
            ...
            .filter(
                ...
                TaskInstance.task_id == "pharma_scrapper_dev",
...
with DAG('pharma_data_full_pipeline_dev',
    ...
    airbyte_mongo_to_postgre = AirbyteTriggerSyncOperator(
        task_id='airbyte_mongo_to_postgre_dev',
    ...
    dbt_seed_dev = create_cloud_run_task_dbt("seed", "seed", "dev")
    dbt_snapshot_dev = create_cloud_run_task_dbt("snapshot", "snapshot", "dev")
    dbt_run_dev = create_cloud_run_task_dbt("run", "run", "dev", exclude_model="staging.curated.stg_similarity_scores_categorie_taxonomy")
    dbt_test_dev = create_cloud_run_task_dbt("test", "test", "dev")
    ...
    airbyte_postgre_to_firestore = AirbyteTriggerSyncOperator(
        task_id='airbyte_postgre_to_firestore_dev',
        airbyte_conn_id='airbyte_conn',
    ...
    save_sitemaps_links_to_mongo >> get_exec_date >> clean_exec_date >> run_pharma_scrapper >> airbyte_mongo_to_postgre >> dbt_seed_dev >> dbt_snapshot_dev >> dbt_run_dev >> dbt_test_dev >> run_save_images >> airbyte_postgre_to_firestore
